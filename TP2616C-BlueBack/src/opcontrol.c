...
//#define desired 89
 ...
 void flywheel_go(float speed){
  ...
  motor_set_vel_pid(MOTOR_FLYWHEEL, pid);
  motor_move_velocity(MOTOR_FLYWHEEL,speed);
}
...
// {controller_rumble(CONTROLLER_MASTER, "-.-");}
  ...
  motor_set_reversed(2, 0);
 		...
 		flywheel_go(120);
	while(true) {
		  motor_set_reversed(11, 0);
  motor_set_reversed(3, 0);
    int power = controller_get_analog(E_CONTROLLER_MASTER, E_CONTROLLER_ANALOG_RIGHT_X);
    int turn = controller_get_analog(E_CONTROLLER_MASTER, E_CONTROLLER_ANALOG_LEFT_Y);
    int left = power + turn;
    int right = power - turn;
    right *= -1; // This reverses the right motor
    motor_move(MOTOR_DRIVE_FRONT_LEFT, left);
	  motor_move(MOTOR_DRIVE_BACK_LEFT, left);
    motor_move(MOTOR_DRIVE_BACK_RIGHT ,right);
	 motor_move(MOTOR_DRIVE_FRONT_RIGHT, right);
		...
		intakeForward = controller_get_digital(E_CONTROLLER_MASTER, E_CONTROLLER_DIGITAL_A);
		intakeBackward = controller_get_digital(E_CONTROLLER_MASTER, E_CONTROLLER_DIGITAL_R2);

    indexerForward = controller_get_digital(E_CONTROLLER_MASTER, E_CONTROLLER_DIGITAL_X);
    indexerBackward = controller_get_digital(E_CONTROLLER_MASTER, E_CONTROLLER_DIGITAL_R1);

		descorerForward = controller_get_digital(E_CONTROLLER_MASTER, E_CONTROLLER_DIGITAL_L2);
    descorerBackward = controller_get_digital(E_CONTROLLER_MASTER, E_CONTROLLER_DIGITAL_L1);
