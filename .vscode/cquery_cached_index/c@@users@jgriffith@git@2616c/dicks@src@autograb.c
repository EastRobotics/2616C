...
extern Semaphore AutoStackSema, downSema;
...
void autoGrab2(void *ignore) {
  ...
  while (true) {
    ...
    motorSet(1, -30);
    motorSet(3, 120);
    while (encoderGet(swingEnc) > swingE - 90) {
    };
    motorSet(3, 0);
...
void autodown(void *ignore) {
  while (1 == 1) {
    semaphoreTake(downSema, 1000000);
    int lifttop = encoderGet(liftEnc);
    printf("Autodown running\n");
    motorSet(1, 127);
    delay(300);
    motorSet(2, 127);
    motorSet(6, 127);
    while (encoderGet(liftEnc) < lifttop + 10) {printf("Lift Encoder - %d  Lift top - %d  lift stop - %d\n",encoderGet(liftEnc), lifttop , lifttop + 10);
      delay(10);
    }
    motorSet(1, 0);
    motorSet(2, 0);
    motorSet(6, 0);
    motorSet(3, -110);
    printf("Swing Limit Switch: %s\n", (digitalRead(9) == LOW)?"Pressed":"Released");
    while (!swinglimit) {
      delay(10);
      printf("Autodown swing down loop\n");
    } // encoderGet(swingEnc) < swingdownset
    motorSet(3, 0);
    motorSet(2, -127);
    motorSet(6, -127);
    while (encoderGet(liftEnc) > liftdownset + 10) {
      delay(10);
      printf("Autodown Lift Down loop\n");
    }
    motorSet(2, 0);
    motorSet(6, 0);
    motorSet(1, -90);
